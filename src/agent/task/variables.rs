...
pub(crate) fn parse_pre_defined_values(defines: &Vec<String>) -> Result<()> {
...
pub(crate) fn interpolate_variables(expr: &str) -> Result<String> {
...
pub(crate) fn parse_variable_expr(expr: &str) -> Result<(String, String)> {
    ...
    let var_value = if let Ok(value) = std::env::var(var_name) {
        ...
        // get from user
        let var_value = cli::get_user_input(&format!("\nplease set ${}: ", var_name.yellow()));
